...
def generate_pdf_playwright(url: str, out_path: Path, timeout: int = 30000, page_selector: Optional[str] = None) -> bool:
    try:
        ...
        with sync_playwright() as pw:
            ...
            # allow targeting a specific page container (default PageContainer3)
            selector = page_selector or '#PageContainer3'
            # Ensure the selector uses sensible background sizing
            page.add_style_tag(content=f"{selector} {{ background-position: center !important; background-repeat: no-repeat !important; background-size: contain !important; }}")

            # If the page container has a background-image, force it to cover and ensure white backdrop
            try:
                bg = page.evaluate(f"() => {{ const el = document.querySelector('{selector}'); return el ? window.getComputedStyle(el).backgroundImage : null; }}")
            except Exception:
                bg = None
            if bg and bg != 'none':
                try:
...
def generate_image_playwright(url: str, out_path: Path, img_format: str = "png", timeout: int = 30000, page_selector: Optional[str] = None) -> bool:
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            # Prepare screenshot kwargs
            screenshot_kwargs = {"path": str(out_path)}
            if img_format.lower() in ("jpeg", "jpg"):
                screenshot_kwargs.update({"type": "jpeg", "quality": 90, "full_page": False})
            else:
                screenshot_kwargs.update({"type": "png", "full_page": False})

            if clip:
                # Playwright expects clip to have float values; construct accordingly
                screenshot_kwargs.update({"clip": {"x": float(clip["x"]), "y": float(clip["y"]), "width": float(clip["width"]), "height": float(clip["height"])}})
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    for u in gen:
        ...
        # optionally generate PDF and/or image
        ...
        if args.to_pdf:
            ...
            if args.skip_existing and out_path.exists():
                ...
                success = False
                if backend == "playwright":
                    success = generate_pdf_playwright(u, out_path, page_selector=page_selector)
        ...
        if args.to_png:
            ...
            if args.skip_existing and img_path.exists():
                ...
                img_success = generate_image_playwright(u, img_path, img_format=args.img_format, page_selector=page_selector)
                if img_success:
